...
public final class Crypt {
    ...
    static void trace(boolean trace, String msg) {
        if (DEBUG) {
            print(msg);
        }
    }
    ...
    final int BLOCK_SIZE = 16; // default block size in bytes
    final int[] alog = new int[256];
    final int[] log = new int[256];
    final byte[] S = new byte[256];
    final byte[] Si = new byte[256];
    final int[] T1 = new int[256];
    final int[] T2 = new int[256];
    final int[] T3 = new int[256];
    final int[] T4 = new int[256];
    final int[] T5 = new int[256];
    final int[] T6 = new int[256];
    final int[] T7 = new int[256];
    final int[] T8 = new int[256];
    final int[] U1 = new int[256];
    final int[] U2 = new int[256];
    final int[] U3 = new int[256];
    final int[] U4 = new int[256];
     ...
     */
    public Object makeKey(byte[] k) throws InvalidKeyException {
     ...
     * @exception  InvalidKeyException  If the key is invalid.
     */
    public synchronized Object makeKey(byte[] k, int blockSize)
     ...
     * @return The ciphertext generated from a plaintext using the session key.
     */
    private byte[] blockEncrypt(byte[] in, int inOffset, Object sessionKey, int blockSize) {
     ...
     * @return The plaintext generated from a ciphertext using the session key.
     */
    private byte[] blockDecrypt(byte[] in, int inOffset, Object sessionKey, int blockSize) {
        if (blockSize == BLOCK_SIZE) {
    ...
    private boolean self_test(int keysize) {
        ...
        try {
            ...
            if (DEBUG) {
                trace(true, "Exception encountered during self-test: " + x.getMessage());
                x.printStackTrace();
            }
        }
        if (DEBUG) {
            trace(true, "Self-test OK? " + ok);
        }
        if (DEBUG) {
            trace(OUT, "self_test()");
        }
    ...
    /**
    Pad the plainText to 128bit blocksize.
    Encrypt w/ secretKey and return encryptedArray
     **/
    public byte[] encrypt(byte[] plainText, byte[] secretKey) throws InvalidKeyException {
        Object key = makeKey(secretKey, BLOCK_SIZE);

        // create a buffer that is padded to the 128 bit boundary (16 byte)
        int paddedLength = plainText.length;
        while (paddedLength % 16 > 0) {
            paddedLength++;
        }

        byte[] paddedPlainText = new byte[paddedLength]; //for system array copy later
        System.arraycopy(plainText, 0, paddedPlainText, 0, plainText.length);

        if (DEBUG) {
            System.out.println("\npaddedPlainText:");
            for (int i = 0; i < paddedPlainText.length; i++) {
                System.out.print(" 0x" + Integer.toHexString(paddedPlainText[i] & 0xFF));
            }
        }
        ...
        if (DEBUG) {
            System.out.println("blockCount.len " + blockCount);
        }

        for (int i = 0; i < blockCount; i++) {
            byte[] plainBlock = new byte[16];
            System.arraycopy(paddedPlainText, i * 16, plainBlock, 0, 16);

            byte[] cipherBlock = blockEncrypt(plainBlock, 0, key, BLOCK_SIZE);
            System.arraycopy(cipherBlock, 0, cipherText, i * 16, 16);
        }

        if (DEBUG) {
            System.out.println("\ncipherText:");
            for (int i = 0; i < cipherText.length; i++) {
                System.out.print(" 0x" + Integer.toHexString(cipherText[i] & 0xFF));
            }
        }
        return cipherText;
    }
    ...
    /**
    Decrypt the cipherText with the secretKey
     **/
    public byte[] decrypt(byte[] cipherText, byte[] secretKey) throws InvalidKeyException {
        Object key = makeKey(secretKey, BLOCK_SIZE);
        ...
        for (int i = 0; i < blockCount; i++) {
            ...
            System.arraycopy(cipherText, i * 16, cipherBlock, 0, 16);

            byte[] plainBlock = blockDecrypt(cipherBlock, 0, key, BLOCK_SIZE);
            System.arraycopy(plainBlock, 0, plainText, i * 16, 16);
        }
